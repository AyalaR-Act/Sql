-- Limpieza de datos con lenguaje sql
-- Anotaciones: Cargar el archivo bd1 y crear una base de datos 
-- La base de datos sobre la que se trabajó en clean 
CREATE DATABASE clean;
use clean;
-- Creamos un proceso almacenado para visualiazar la tabla de manera mas eficiente
DELIMITER //
...
-- Realizamos las modificaciones correspondientes para eliminar los espacios
call alldata();
...
-- Notemos que no se eliminaron todos los registros, verifiquemos si existen duplicados nuevamente
...
group by id_cliente
having count(*) > 1;
-- Notemos que los duplicados son los que tienen el identificador (Id_cliente) 
-- Es importante solucionar este error puesto que para ser llave primaria deberá cumplir el que id_cliente sea unico para cada registro
-- Los id_cliente duplicados son :  4412, 2189, 2376, 7540
-- Tenemos un total de 4 duplicados evaluemos la causa raíz realizamos la consulta
SELECT *
FROM dataorganization
WHERE id_cliente IN (4412, 2189, 2376, 7540);
...
-- Ahora modificamos el id_cliente para los registros identificados 
-- Se hará un cambio en el id_cliente a ciertos registros y lo haremos por la columna nombres
call alldata();
UPDATE dataorganization
...
-- Modifiquemos ahora la columna salary
-- Asignemos datos nulos en cuadros vacios
UPDATE dataorganization 
SET salary = NULL
WHERE salary IS NULL OR salary = '' OR TRIM(salary) = '';
-- Eliminamos el simbolo de pesos u otro factor que no permita tener el registro como tipo de dato int o double
...
-- Asignamos un tipo de dato int a la  columna salary
